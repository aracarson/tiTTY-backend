...
impl RateLimiter {
    pub fn check(&self, key: &str, limit: u32, window: Duration) -> bool {
         ...
         */
        buckets.retain(|_, bucket| now.duration_since(bucket.started) < Duration::from_secs(3_600));

        /*
         * Refuse to allocate additional buckets
         * after reaching the safety limit.
         */
        if !buckets.contains_key(key) && buckets.len() >= 4_096 {
            return false;
        }
...
pub fn enforce(
     ...
     * same concrete type.
     */
    let limiter = ctx.data::<Arc<RateLimiter>>()?;
...
mod tests {
    ...
    fn isolates_rate_limits_by_key() {
        ...
        assert!(limiter.check("client-a:register", 1, Duration::from_secs(60),));

        assert!(limiter.check("client-b:register", 1, Duration::from_secs(60),));
    }
    ...
    fn isolates_rate_limits_by_scope() {
        ...
        assert!(limiter.check("client:challenge", 1, Duration::from_secs(60),));
    }
    ...
    fn zero_limit_rejects_request() {
        ...
        assert!(!limiter.check("client:register", 0, Duration::from_secs(60),));
    }
}
